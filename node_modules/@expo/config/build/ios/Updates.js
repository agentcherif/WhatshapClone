...
function formatConfigurationScriptPath(projectDir, config) {
    const buildScriptPath = Modules_1.projectHasModule('expo-updates/scripts/create-manifest-ios.sh', projectDir, config);
    if (!buildScriptPath) {
    ...
    return path_1.default.relative(path_1.default.join(projectDir, 'ios'), buildScriptPath);
}
...
function ensureBundleReactNativePhaseContainsConfigurationScript(projectDir, config, project) {
...
function isShellScriptBuildPhaseConfigured(projectDir, config, project) {
    const bundleReactNative = getBundleReactNativePhase(project);
    const buildPhaseShellScriptPath = formatConfigurationScriptPath(projectDir, config);
