...
function formatApplyLineForBuildGradle(projectDir, config) {
    const updatesGradleScriptPath = Modules_1.projectHasModule('expo-updates/scripts/create-manifest-android.gradle', projectDir, config);
    ...
    return `apply from: ${JSON.stringify(path_1.default.relative(path_1.default.join(projectDir, 'android', 'app'), updatesGradleScriptPath))}`;
}
exports.formatApplyLineForBuildGradle = formatApplyLineForBuildGradle;
function isBuildGradleConfigured(buildGradleContent, projectDir, config) {
...
function isMainApplicationMetaDataSet(manifest) {
    const updateUrl = Manifest_1.getMainApplicationMetaDataValue(manifest, Config.UPDATE_URL);
    const runtimeVersion = Manifest_1.getMainApplicationMetaDataValue(manifest, Config.RUNTIME_VERSION);
    const sdkVersion = Manifest_1.getMainApplicationMetaDataValue(manifest, Config.SDK_VERSION);
    return Boolean(updateUrl && (sdkVersion || runtimeVersion));
}
exports.isMainApplicationMetaDataSet = isMainApplicationMetaDataSet;
function isMainApplicationMetaDataSynced(config, manifest, username) {
    return (getUpdateUrl(config, username) ===
        Manifest_1.getMainApplicationMetaDataValue(manifest, Config.UPDATE_URL) &&
        String(getUpdatesEnabled(config)) ===
            Manifest_1.getMainApplicationMetaDataValue(manifest, Config.ENABLED) &&
        String(getUpdatesTimeout(config)) ===
            Manifest_1.getMainApplicationMetaDataValue(manifest, Config.LAUNCH_WAIT_MS) &&
        getUpdatesCheckOnLaunch(config) ===
            Manifest_1.getMainApplicationMetaDataValue(manifest, Config.CHECK_ON_LAUNCH) &&
        areVersionsSynced(config, manifest));
}
exports.isMainApplicationMetaDataSynced = isMainApplicationMetaDataSynced;
function areVersionsSynced(config, manifest) {
    const expectedRuntimeVersion = getRuntimeVersion(config);
    const expectedSdkVersion = getSDKVersion(config);
    const currentRuntimeVersion = Manifest_1.getMainApplicationMetaDataValue(manifest, Config.RUNTIME_VERSION);
    const currentSdkVersion = Manifest_1.getMainApplicationMetaDataValue(manifest, Config.SDK_VERSION);
